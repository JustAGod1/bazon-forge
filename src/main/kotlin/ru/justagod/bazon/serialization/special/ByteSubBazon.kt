...
object ByteSubBazon : SubBazon {
    ...
    override fun deserialize(model: TypeReference, appender: BytecodeAppender, context: DeserializationContext, parent: AbstractModel) {
        appender += NewInstanceInstruction(ru.justagod.bazon.serialization.special.ByteSubBazon.type)
        appender += DupInstruction(ru.justagod.bazon.serialization.special.ByteSubBazon.type)
        context.reader.readByte(appender)
        appender += InvokeInstanceMethodInstruction(ru.justagod.bazon.serialization.special.ByteSubBazon.type, "<init>", "(B)V", Opcodes.INVOKESPECIAL)
    }
    ...
    override fun serialize(model: TypeReference, appender: BytecodeAppender, context: SerializationContext, parent: AbstractModel) {
        ...
        context.workWithTop(ru.justagod.bazon.serialization.special.ByteSubBazon.type, appender) {
            update(valueBuilder)
            valueBuilder += InvokeInstanceMethodInstruction(ru.justagod.bazon.serialization.special.ByteSubBazon.type, "byteValue", "()B", Opcodes.INVOKEVIRTUAL)
        }
