...
class BytecodeStack private constructor(private val content: Stack<TypeReference>) {
    ...
    fun removeValue(type: TypeReference) {
        if (content.empty()) error("Type stack is empty:(")
        val top = content.peek()
        if (top != type) error("Illegal type on top of the stack.\nExpected: $type. Actual: $top")
        content.pop()
    }

    fun copy(): ru.justagod.bazon.stackManipulation.BytecodeStack = ru.justagod.bazon.stackManipulation.BytecodeStack(content.copy())
